...
Pure functions extracted from ``EnergyConverterRole``; no mango context needed.
"""
...
def aggregate_flex_answers(answers: list[AvailableFlexAnswer]) -> FlexAggregate:
    ...
    Tracks signed balance, unsigned unmet (so disconnected loads don't cancel
    against generation), and the lowest-tier unmet pair so the ADMM weight is
    top-tier-dominant rather than magnitude-weighted.
    """
    agg = FlexAggregate({}, {}, {}, {}, {})
    for answer in answers:
        ...
        for sec_str, val in (getattr(answer, "unmet_by_sector", {}) or {}).items():
            try:
                ...
                continue
            agg.unmet_by_sector_total[sec_enum] = agg.unmet_by_sector_total.get(
                sec_enum, 0.0
            ) + float(val)
...
def compute_sector_priorities(np, agg: FlexAggregate):
    """Top-tier-dominant priority weights for the ADMM sharing problem.

    A sector whose lowest unmet tier is t outranks any with top tier > t;
    ...
    def _sector_w(sec: Sector) -> float:
        ...
        # Strict-monotone: tier 1 gets the highest weight. Uses the L2/L3 helper,
