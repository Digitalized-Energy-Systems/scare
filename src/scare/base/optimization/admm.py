...
Replaces the upstream deny-list message filter with an allow-list of the ADMM
message types, so scare-domain messages routed to a CP agent pass through
untouched instead of crashing the ADMM handler.
"""
...
class ScareDistributedOptimizationRole(DistributedOptimizationRole):
    ...
    def _handle_optimization(self, content: Any, meta: dict) -> None:
        ...
        # Route through mango's scheduler, not bare asyncio.create_task: under
        # discrete-stepping an orphan task runs off the simulation clock.
