...
Each neighbour pair carries a continuous reliability score K(t) in [0, 1]
that replaces the binary BROKEN/operational flag.  The score recovers
multiplicatively on message arrivals and decays in their absence:
...
The score feeds the live-neighbour predicate (K >= tau), biases gossip
next-hop selection toward high-K neighbours, and weights reported
neighbour utilisation by the K-score of the arriving link.
"""
...
# Per-second silence decay and per-message recovery rates.  Tuned so:
# 1 missed second drops K by ~0.1, one reception recovers a moderately
# decayed K, and ~8 s of silence drives K below the 0.5 liveness threshold.
...
class TrustLedger:
    ...
    def score(self, addr_key: str, now: float) -> float:
        ...
        if entry is None:
            # Unknown neighbours start at the prior (fully trusted on first
            # contact: no elapsed time, so no decay).
    ...
    def on_silence(self, addr_key: str, now: float) -> float:
        ...
        if addr_key in self._broken:
            return _MIN_K
        prior = self.score(addr_key, now)
        # score() already applied decay; persist it so later calls see the
        # lower value without compounding it.
    ...
    def clear_broken(self, addr_key: str, now: float) -> None:
        ...
        cautious half-initial value rather than 1.0."""
    ...
    def _decay(self, k: float, dt: float) -> float:
        ...
        # Linear approximation of exp(-decay_rate_per_s * dt) * k, clamped to [0, 1].
...
def hash_weighted_choice(
    ...
    Picks the index whose cumulative weight covers the SHA256-derived
    fraction in [0, total): identical inputs always yield the same output,
    and high-weight addresses are picked proportionally more often.

    Returns None for empty inputs; falls back to a hash-uniform pick if all
