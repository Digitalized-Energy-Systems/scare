...
def test_envelope_clamp_overwrites_only_present_sectors():
    env = CoalitionEnvelope()
    env.set({Sector.ELECTRICITY: 5.0, Sector.GAS: -2.0}, ttl_s=10.0,
            coalition_id="c1", now=0.0)
    result = [1.0, 2.0, 3.0]
...
def test_aggregate_sums_balance_and_unmet_per_sector():
    answers = [
        _answer(Sector.ELECTRICITY, balance=2.0,
                unmet_by_sector={"electricity": 1.0}),
        _answer(Sector.ELECTRICITY, balance=-0.5,
                unmet_by_sector={"electricity": 0.5}),
    ]
...
def test_compute_cp_setpoint_zero_capacity_or_no_ratios():
    ...
    assert compute_cp_setpoint(
        {"capacity_mw": 5.0, "coupling_ratios": {}}, {}) == {}


def test_compute_cp_setpoint_runs_when_output_more_stressed():
    meta = {"capacity_mw": 4.0,
            "coupling_ratios": {("electricity", "heat"): 1.0}}
