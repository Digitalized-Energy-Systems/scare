...
from scare.service.heat_frontier import FrontierDecision, HeatFrontierController
...
def _willingness(tier, cap, reducible, *, sens=0.01, sens_ref=0.01):
    return curtail_willingness(
        priority_tier=tier, capacity=cap, reducible=reducible,
        sensitivity=sens, sensitivity_ref=sens_ref, priority_tiers=4,
        sens_mult_min=_SENS_MIN, sens_mult_max=_SENS_MAX,
    )
...
def test_allocation_proportional_to_willingness():
    ...
    plan = plan_auction_allocation(bids, bidders, {}, 1.0, waterfall=False,
                                   min_reducible=5e-4)
...
def test_waterfall_sheds_lowest_priority_tier_first_full_amount():
    ...
    plan = plan_auction_allocation(bids, bidders, bid_meta, 0.8, waterfall=True,
                                   min_reducible=5e-4)
...
def test_waterfall_tier1_exhausted_when_only_tier1_reducible():
    ...
    bid_meta = {"crit": (1, 0.5)}
    plan = plan_auction_allocation({"crit": 1.0}, {"crit": "a"}, bid_meta, 0.8,
                                   waterfall=True, min_reducible=5e-4)
...
def test_waterfall_ignores_exhausted_reducible():
    ...
    bid_meta = {"lo": (4, 1e-9)}
    plan = plan_auction_allocation({"lo": 1.0}, {"lo": "a"}, bid_meta, 0.8,
                                   waterfall=True, min_reducible=5e-4)
    assert plan.tier1_exhausted is True
...
def test_decide_holds_in_band():
    ...
    out = c.decide(t=318.0, lo=_LO, cap=0.05, cur=1.0, sensitivity=660.0,
                   now=1.0, my_tier=2, has_lock=False, waterfall_enabled=False)
    assert out is None


def test_decide_sheds_when_too_cold():
    c = HeatFrontierController(peer_freshness_s=50.0)
    out = c.decide(t=300.0, lo=_LO, cap=0.05, cur=1.0, sensitivity=660.0,
                   now=1.0, my_tier=2, has_lock=False, waterfall_enabled=False)
...
def test_decide_waterfall_defers_to_lower_priority_peer():
    ...
    out = c.decide(t=300.0, lo=_LO, cap=0.05, cur=1.0, sensitivity=660.0,
                   now=1.0, my_tier=1, has_lock=False, waterfall_enabled=True)
    assert out is None  # deferred while lower-priority reducible remains


def test_decide_restore_requires_lock():
    c = HeatFrontierController(peer_freshness_s=50.0)
    kw = dict(t=325.0, lo=_LO, cap=0.05, cur=0.8, sensitivity=660.0,
              now=1.0, my_tier=2, waterfall_enabled=False)
...
def test_decide_below_commit_threshold_returns_none():
    ...
    out = c.decide(t=314.0, lo=_LO, cap=0.05, cur=1.0, sensitivity=1e6,
                   now=1.0, my_tier=2, has_lock=False, waterfall_enabled=False)
    assert out is None
